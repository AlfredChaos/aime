...
识别尚未被压缩的消息（通过 exclude_mark）
保持最近 keep_recent 条消息不被压缩（以保留最近的上下文）
将较早的消息发送给 LLM 生成结构化摘要
使用 MemoryMark.COMPRESSED 标记已压缩的消息（通过 update_messages_mark）
将摘要存储在记忆中（通过 update_compressed_summary）


重要
压缩采用**标记机制**而非替换消息。旧消息被标记为已压缩，并通过 exclude_mark=MemoryMark.COMPRESSED 在后续检索中被排除，而生成的摘要则单独存储，在需要时检索。这种方式保留了原始消息，允许灵活的记忆管理。关于标记功能的更多详情，请参考 记忆。
...
备注
智能体确保工具使用和工具结果对在压缩过程中保持在一起，以维护对话流程的完整性。


小技巧
可以通过指定不同的 compression_model 和 compression_formatter 来使用更小、更快的模型进行压缩，以降低成本和延迟。
...
备注
ReActAgent 中的工具并行调用是基于异步 asyncio.gather 实现的，因此，只有当工具函数是异步函数，同时工具函数内也为异步逻辑时，才能最大程度发挥工具并行执行的效果


备注
运行时请确保模型层面支持工具并行调用，并且相应参数设置正确（可以通过 generate_kwargs 传入），例如对于DashScope API，需要设置 parallel_tool_calls 为 True，否则将无法进行并行工具调用。
...
Jarvis: {
    "type": "tool_use",
    "id": "call_2bb19eab95c34694a490dd",
...
Jarvis: {
    "type": "tool_use",
    "id": "call_5f9c01d545224248b1cd9f",
    "name": "example_tool_function",
    "input": {
        "tag": "tag2"
    },
...
system: {
    ...
    "output": [
        {
            "type": "text",
            "text": "标签 tag1 开始于 03:05:21.281947，结束于 03:05:24.283067。"
        }
    ]
}
system: {
    ...
    "output": [
        {
            "type": "text",
            "text": "标签 tag2 开始于 03:05:21.282044，结束于 03:05:24.283738。"
        }
    ]
}
Jarvis: 两个 `example_tool_function` 函数已经并行执行完毕。下面是它们的执行时间：

- 标签为 'tag1' 的函数开始于 03:05:21.281947，结束于 03:05:24.283067。
- 标签为 'tag2' 的函数开始于 03:05:21.282044，结束于 03:05:24.283738。

这两个函数几乎是同时开始和结束的，这表明它们确实是以并行的方式执行的。
...
Jarvis: {
    "type": "tool_use",
    "id": "call_66a1b26b095d46a5990b18",
    ...
    "raw_input": "{\"name\": \"阿尔伯特·爱因斯坦\", \"description\": \"20世纪最伟大的理论物理学家之一，相对论的创立者。\", \"age\": 76, \"honor\": [\"诺贝尔物理学奖\", \"普朗克奖章\", \"科普利奖章\"]}"
}
system: {
    "type": "tool_result",
    "id": "call_66a1b26b095d46a5990b18",
...
Jarvis: 阿尔伯特·爱因斯坦是20世纪最伟大的理论物理学家之一，他最为人所知的是作为相对论的创立者。出生于1879年，在其76年的生命历程中，爱因斯坦对物理学界乃至整个科学领域都产生了深远的影响。他的工作不仅限于相对论，在量子力学的发展过程中也扮演了重要角色。

爱因斯坦一生获得了多项荣誉与奖项，其中包括但不限于诺贝尔物理学奖（表彰他在光电效应方面的贡献）、普朗克奖章以及科普利奖章等。这些成就证明了他在科学研究上的卓越贡献及其对后世科学家们的巨大启发作用。
...
Total running time of the script: (0 minutes 32.199 seconds)
